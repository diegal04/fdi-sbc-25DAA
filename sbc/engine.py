...
def unificar(
    patron: Tripleta, hecho: Tripleta, sust_previa: Sustitucion
) -> Sustitucion | None:
    sust = sust_previa.copy()
    for p, h in zip(
        [patron.sujeto, patron.predicado, patron.objeto],
...
class MotorInferencia:
    ...
    def _buscar_combinaciones(
        self, antecedentes: List[Tripleta], sust_actual: Sustitucion
    ) -> Generator[Tuple[Sustitucion, float], None, None]:
        if not antecedentes:
        ...
        for hecho in self.memoria.hechos:
            ...
            if nueva_sust is not None:
                for sust_final, certeza_minima in self._buscar_combinaciones(
                    resto_condiciones, nueva_sust
                ):
    ...
    def encadenamiento_hacia_adelante(self) -> int:
        ...
        while modificado:
            ...
            for regla in self.memoria.reglas:
                for sustitucion, certeza_antecedentes in self._buscar_combinaciones(
                    regla.antecedentes, {}
                ):
    ...
    def encadenamiento_hacia_atras(
        self, objetivo: Tripleta, nivel: int = 0
    ) -> Generator[Tuple[Sustitucion, float], None, None]:
        ...
        for regla in self.memoria.reglas:
            ...
            if sust_regla is not None:
                ...
                for (
                    sust_final,
                    certeza_antecedentes,
                ) in self._verificar_antecedentes_atras(
                    ...
                    vars_objetivo = [
                        t
                        for t in [objetivo.sujeto, objetivo.predicado, objetivo.objeto]
                        if es_variable(t)
                    ]
                    sust_limpia = {}
    ...
    def _verificar_antecedentes_atras(
        self,
        antecedentes: List[Tripleta],
        sust_actual: Sustitucion,
        restricciones: List[Restriccion],
        certeza_acumulada: float,
        nivel: int,
    ) -> Generator[Tuple[Sustitucion, float], None, None]:
        if not antecedentes:
        ...
        for sust_parcial, certeza_parcial in self.encadenamiento_hacia_atras(
            condicion_actual, nivel
        ):
            ...
            yield from self._verificar_antecedentes_atras(
                resto_antecedentes, nueva_sust, restricciones, certeza_minima, nivel
            )
